...
class TestLLMFactory:
    ...
    def test_register_custom_provider(self):
        from app.services.llm.llm_factory import register_provider, LLMFactory
        from app.services.llm.base import BaseLLMProvider

        class DummyProvider(BaseLLMProvider):
            async def generate(self, prompt, **kw): pass
...
class TestPromptBuilder:
    ...
    def test_summary_prompt_structure(self):
        ...
        sys_p, usr_p = pb.build_summary_prompt(
            question="test?", sql="SELECT 1", rows=[{"a": 1}], row_count=1,
        )
        assert "data analyst" in sys_p.lower() or "summarise" in sys_p.lower() or "Summarise" in sys_p
    ...
    def test_summary_prompt_empty_rows(self):
        ...
        _, usr_p = pb.build_summary_prompt(
            question="test?", sql="SELECT 1", rows=[], row_count=0,
        )
...
class TestSchemaContext:
    def test_schema_contains_all_tables(self):
        ...
        for table in ["case_master", "police_station", "crime_type",
                       "accused_master", "victim_master", "financial_transaction", "users"]:
...
class TestExceptions:
    ...
    def test_specific_exceptions(self):
        from app.agents.query_agent.exceptions import (
            SQLGenerationError, SQLValidationError, SQLExecutionError,
            SummaryError, DatabaseUnavailableError, LLMUnavailableError,
        )
...
class TestResponseFormatter:
    ...
    def test_compute_confidence_perfect(self):
        ...
        c = compute_confidence(
            sql_valid=True, rows_returned=10, execution_ok=True, summary_ok=True,
        )
        assert c == 1.0
    ...
    def test_compute_confidence_no_rows(self):
        from app.agents.query_agent.response_formatter import compute_confidence
        c = compute_confidence(
            sql_valid=True, rows_returned=0, execution_ok=True, summary_ok=True,
        )
    ...
    def test_compute_confidence_nothing(self):
        from app.agents.query_agent.response_formatter import compute_confidence
        c = compute_confidence(
            sql_valid=False, rows_returned=0, execution_ok=False, summary_ok=False,
        )
        assert c == 0.0
