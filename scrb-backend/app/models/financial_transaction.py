...
class FinancialTransaction(Base):
    ...
    case_master_id: Mapped[int | None] = mapped_column(
        ForeignKey("case_master.case_master_id", ondelete="SET NULL"),
        index=True
    )
    accused_master_id: Mapped[int | None] = mapped_column(
        ForeignKey("accused_master.accused_master_id", ondelete="SET NULL"),
        index=True
    )

    # Relationships
    case: Mapped["CaseMaster | None"] = relationship(back_populates="financial_transactions")
    accused: Mapped["AccusedMaster | None"] = relationship(back_populates="financial_transactions")
